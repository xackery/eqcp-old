...
func (s *Server) PetitionSearch(ctx context.Context, req *pb.PetitionSearchRequest) (*pb.PetitionSearchResponse, error) {
	...
	query := "SELECT count(id) as total, petitions.* FROM petitions WHERE "

	args := map[string]interface{}{}
	if len(req.Charname) > 0 {
		query += "charname LIKE :charname"
		args["charname"] = fmt.Sprintf("%%%s%%", req.Charname)
	}
